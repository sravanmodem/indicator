...
class DataFetcher:
    ...
    async def get_next_expiry(self, index: str = "NIFTY") -> dict[str, Any]:
        ...
        try:
            ...
            # For BANKNIFTY, filter to monthly expiries only (last Wednesday of each month)
            # BANKNIFTY does NOT have weekly options - only monthly expiry
            if index.upper() == "BANKNIFTY":
                monthly_expiries = []
                for expiry in future_expiries:
                    # Check if this is the last expiry in its month
                    # Monthly expiry = no other expiry in the same month after this one
                    is_monthly = True
                    for next_exp in future_expiries:
                        if next_exp > expiry and next_exp.month == expiry.month and next_exp.year == expiry.year:
                            is_monthly = False
                            break
                    if is_monthly:
                ...
                if monthly_expiries:
                    ...
                    logger.info(f"BANKNIFTY monthly expiries: {[str(e) for e in monthly_expiries[:3]]}")
