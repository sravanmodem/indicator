...
- Trading hours restriction (9:15 AM - 3:30 PM)
- Weekend and holiday detection from Kite
"""
...
# This is a fallback - Kite's is_market_open() is preferred
INDIAN_MARKET_HOLIDAYS = {
    ...
    date(2026, 5, 12),   # Buddha Purnima
    date(2026, 6, 5),    # Bakrid
...
class SignalContext:
    ...
    # 15-min of 1-min OHLCV data (short-term momentum)
    ohlcv_1min: list[dict] = field(default_factory=list)

    # 1-hour of 10-min OHLCV data (longer-term trend)
    ohlcv_10min: list[dict] = field(default_factory=list)
    ...
    pivot_points: dict = field(default_factory=dict)  # S1, S2, R1, R2, PP
    volume_profile: dict = field(default_factory=dict)  # POC, VAH, VAL
    atr: float = 0.0  # Average True Range for volatility
...
class AITradingService:
    ...
    def is_trading_hours(self) -> tuple[bool, str]:
        """
        Check if AI trading is allowed based on:
        - Weekday (Mon-Fri)
    ...
    async def check_market_from_kite(self) -> tuple[bool, str]:
        """
        Check market status using Kite API (more reliable).
        Falls back to time-based check if Kite unavailable.
        """
        try:
            from app.services.zerodha_auth import get_auth_service
            auth = get_auth_service()

            if not auth.is_authenticated or not auth.kite:
                # Fall back to time-based check
                return self.is_trading_hours()

            # Try to get instruments list - if it works, market is accessible
            # Kite API will throw error if market is closed for maintenance
            kite = auth.kite

            # Check if we can fetch quotes (if market is open)
            try:
                # Just check if LTP call works
                quotes = await asyncio.to_thread(kite.ltp, ["NSE:NIFTY 50"])
                if quotes:
                    return True, "Market is open (verified via Kite)"
            except Exception as kite_err:
                error_str = str(kite_err).lower()
                if "market" in error_str and "closed" in error_str:
                    return False, "Market closed (from Kite API)"
                if "holiday" in error_str:
                    return False, "Market holiday (from Kite API)"
                # Other error - fall back to time check
                return self.is_trading_hours()

        except Exception as e:
            logger.warning(f"Kite market check failed, using time-based: {e}")
            return self.is_trading_hours()
    ...
    async def analyze_entry_signal(self, context: SignalContext) -> AITradeDecision:
        ...
        is_open, reason = await self.check_market_from_kite()
        if not is_open:
            logger.info(f"AI SKIP: {reason}")
            return AITradeDecision(
                action="SKIP",
                confidence=0,
                reasoning=f"Market not open: {reason}",
            )
    ...
    def _build_entry_prompt(self, ctx: SignalContext) -> str:
        ...
        ohlcv_1min = ctx.ohlcv_1min if ctx.ohlcv_1min else ctx.ohlcv_data
        ohlcv_10min = ctx.ohlcv_10min if ctx.ohlcv_10min else []

        ohlcv_1min_summary = self._format_ohlcv(ohlcv_1min)
        ohlcv_10min_summary = self._format_ohlcv(ohlcv_10min) if ohlcv_10min else "No 10-min data available"
...
"""
        ...
        # Build pivot points section if available
        pivot_section = ""
        if ctx.pivot_points:
            pivot_section = f"""
## PIVOT POINTS (Support/Resistance)
- Pivot Point (PP): {ctx.pivot_points.get('pp', 0):.2f}
- Resistance 1 (R1): {ctx.pivot_points.get('r1', 0):.2f}
- Resistance 2 (R2): {ctx.pivot_points.get('r2', 0):.2f}
- Support 1 (S1): {ctx.pivot_points.get('s1', 0):.2f}
- Support 2 (S2): {ctx.pivot_points.get('s2', 0):.2f}
"""

        return f"""You are an expert Indian options trading AI with 15+ years experience in NIFTY/BANKNIFTY/SENSEX options. Analyze this signal and decide BUY or SKIP.
...
{expiry_warning}
## SHORT-TERM MOMENTUM - Last 15 Minutes (1-min candles)
{ohlcv_1min_summary}

## LONGER-TERM TREND - Last 1 Hour (10-min candles)
{ohlcv_10min_summary}
...
- ATR (Volatility): {ctx.atr:.2f}
- Bollinger: Upper={ctx.bollinger.get('upper', 0):.2f}, Middle={ctx.bollinger.get('middle', 0):.2f}, Lower={ctx.bollinger.get('lower', 0):.2f}
{pivot_section}
## OPTION GREEKS
- Delta: {ctx.delta:.3f} ({ctx.delta * 100:.0f}% directional exposure)
- Gamma: {ctx.gamma:.5f} ({"HIGH RISK - rapid delta changes!" if ctx.gamma > 0.01 else "Normal"})
- Theta: Rs.{ctx.theta:.2f}/day (daily time decay)
- Vega: Rs.{ctx.vega:.2f}/1% IV change
- IV (Implied Volatility): {ctx.iv:.1f}%
...
## YOUR ANALYSIS TASK
1. Check SHORT-TERM momentum (1-min data):
   - Recent price action direction
   - Immediate support/resistance levels
   - Entry timing confirmation

2. Confirm with LONGER-TERM trend (10-min data):
   - Overall trend direction
   - Major support/resistance zones
   - Trend strength and momentum

3. Evaluate INDICATOR ALIGNMENT:
   - Is trend (SuperTrend + EMA) supporting signal direction?
   - Is momentum (RSI, Stochastic, MACD) confirming?
   - Is ADX showing strong trend?

4. Assess RISK via Greeks:
   - High gamma = volatile (especially on expiry day)
   - High theta = time decay hurting position
   - Check Risk:Reward ratio

5. DECISION:
   - BUY only if multiple confirmations align
   - SKIP if signals conflict or risk is high
   - Optionally adjust SL/Target to better levels

RESPOND IN STRICT JSON FORMAT ONLY:
{{"action": "BUY" or "SKIP", "confidence": 0-100, "reasoning": "2-3 sentence explanation covering both timeframes", "adjusted_target": null or number, "adjusted_stop_loss": null or number}}
"""
